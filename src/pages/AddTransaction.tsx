  ...
  return (
    <div className="min-h-screen bg-[#0a0e1a] flex justify-center">
      ...
      <div className="w-full max-w-7xl mx-auto px-5 lg:px-8">
        {/* Header */}
        <div className="flex items-center justify-between px-2 pt-4 h-14">
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
              ...
              <div className="relative">
                <select
                  value={fromAccount}
                  onChange={(e) => setFromAccount(e.target.value)}
                  className="w-full h-14 px-4 rounded-2xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-emerald-500/50 focus:bg-white/[0.07] transition-all appearance-none cursor-pointer"
                >
                  {/* Dropdown UI fix using Tailwind classes on option tags */}
                  <option value="" className="bg-[#111827] text-slate-400">Select account</option>
                  {accounts.map((account) => (
                    <option
                      key={account.id}
                      value={account.id}
                      className="bg-[#111827] text-white py-3 option-style"
                    >
                      {account.name} • ${account.balance.toLocaleString()}
                    </option>
                  ))}
                </select>
                {/* Custom Custom Arrow Icon for modern look */}
                ...
                >
                  ...
                  {accounts.map((account) => (
                    <option
                      key={account.id}
                      value={account.id}
                      className="bg-[#111827] text-white py-3"
                    >
                      {account.name} • ${account.balance.toLocaleString()}
                    </option>
                  ))}
